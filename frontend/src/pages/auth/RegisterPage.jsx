...
/* ─────────────────────────────────────────────
   GLOBAL CSS (same as LoginPage — idempotent)
───────────────────────────────────────────────*/
const GLOBAL_CSS = `
...
function RegisterPage() {
  ...
  useEffect(() => {
    ...
    return () => { const el = document.getElementById('auth-css'); if (el) el.remove(); };
  }, []);
  ...
  const PERKS = [
    { icon: '🍣', title: 'Exclusive Menu Access',   desc: 'Order seasonal chef specials before anyone else.' },
    { icon: '📅', title: 'Priority Reservations',   desc: 'Skip the queue with member-first table booking.' },
    { icon: '🎁', title: 'Member Rewards',           desc: 'Earn points on every order and redeem for perks.' },
    { icon: '⚡', title: 'Express Delivery',         desc: 'Guaranteed delivery in 30 minutes or less.' },
  ];
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="reg-left" style={{
        ...
        background: 'linear-gradient(145deg, #0f0f0f 0%, #1a0a04 60%, #0a0a0a 100%)',
        borderRight: '1px solid rgba(255,255,255,0.05)',
      }}>
        {/* Glow orbs */}
        <div style={{ position: 'absolute', top: '15%', right: '-80px', width: '340px', height: '340px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(232,68,26,0.14) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: '15%', left: '-60px', width: '260px', height: '260px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(232,68,26,0.07) 0%, transparent 70%)', pointerEvents: 'none' }} />

        {/* Logo */}
        <div style={{ marginBottom: '48px' }}>
          <a href="/" style={{ textDecoration: 'none' }}>
            <div style={{ fontSize: '22px', fontFamily: "'Playfair Display', serif", fontWeight: '700', letterSpacing: '2px', color: '#fff', marginBottom: '6px' }}>
              <span style={{ color: '#E8441A' }}>美食家大廳</span> Epicure <span style={{ color: '#E8441A' }}>Hall</span>
            </div>
          </a>
          <div style={{ width: '40px', height: '2px', background: '#E8441A' }} />
        </div>

        {/* Headline */}
        <div style={{ marginBottom: '40px' }}>
          <div style={{ fontSize: '12px', fontWeight: '700', letterSpacing: '4px', textTransform: 'uppercase', color: '#E8441A', marginBottom: '16px' }}>
            Join the Experience
          </div>
          <h1 style={{ fontSize: 'clamp(30px, 3vw, 46px)', fontFamily: "'Playfair Display', serif", fontWeight: '700', color: '#fff', lineHeight: '1.12', marginBottom: '16px' }}>
            Start Your<br />
            <span style={{ color: '#E8441A', fontStyle: 'italic' }}>Culinary Journey</span>
          </h1>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.5)', lineHeight: '1.7', maxWidth: '340px' }}>
            Create a free account and unlock exclusive dining privileges at Epicure Hall.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {PERKS.map((p, i) => (
            <div key={p.title} className="feature-item" style={{ animationDelay: `${0.4 + i * 0.08}s`, display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(232,68,26,0.1)', border: '1px solid rgba(232,68,26,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px', flexShrink: 0, marginTop: '2px' }}>
                {p.icon}
              </div>
              <div>
                <div style={{ fontSize: '14px', fontWeight: '600', color: 'rgba(255,255,255,0.85)', marginBottom: '2px' }}>{p.title}</div>
      ...
      <div className="reg-right" style={{
        ...
        background: '#0e0e0e', overflowY: 'auto',
      }}>

        {/* Form header */}
        <div style={{ marginBottom: '28px' }}>
          <div className="logo-pulse" style={{ fontSize: '38px', marginBottom: '14px' }}>🍽️</div>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: '26px', fontWeight: '700', color: '#fff', marginBottom: '8px' }}>
            Create Account
          </h2>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div className="form-row-0" style={{ marginBottom: '14px' }}>
            ...
            <input type="text" placeholder="Ali Khan" value={formData.full_name}
          ...
          <div className="form-row-3" style={{ marginBottom: '14px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPass ? 'text' : 'password'} placeholder="Min 8 characters"
                ...
                className="auth-input" style={{ paddingRight: '48px' }} required />
          ...
          <div className="form-row-4" style={{ marginBottom: '6px' }}>
            ...
            {/* live match feedback */}
            {pwMatch === false && (
              <p style={{ fontSize: '12px', color: '#ff4d4d', marginTop: '6px', fontFamily: "'DM Sans',sans-serif" }}>
                ✕ Passwords do not match
              </p>
            )}
            {pwMatch === true && (
              <p style={{ fontSize: '12px', color: '#4ade80', marginTop: '6px', fontFamily: "'DM Sans',sans-serif" }}>
                ✓ Passwords match
              </p>
            )}
          </div>
          ...
          <div className="form-row-5" style={{ marginTop: '22px' }}>
            <button type="submit" disabled={loading || pwMatch === false} className="auth-btn-primary">
              {loading ? (
        ...
        {/* Divider */}
        <div className="form-row-6" style={{ display: 'flex', alignItems: 'center', margin: '24px 0' }}>
          <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
          <span style={{ padding: '0 16px', fontSize: '11px', fontWeight: '600', letterSpacing: '2px', color: 'rgba(255,255,255,0.22)', textTransform: 'uppercase' }}>or sign up with</span>
          <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
        </div>

        {/* Social buttons (original handlers) */}
        <div className="form-row-7" style={{ display: 'flex', gap: '12px' }}>
          <button onClick={loginWithGoogle} className="social-btn" style={{ background: '#fff', color: '#111' }}>
          ...
          <button onClick={loginWithFacebook} className="social-btn" style={{ background: '#1877F2', color: '#fff' }}>
        ...
        {/* Login link */}
        <p style={{ textAlign: 'center', marginTop: '28px', fontSize: '14px', color: 'rgba(255,255,255,0.35)' }}>
          Already a member?{' '}
          <Link to="/login" className="auth-link">Sign in →</Link>
        </p>
      </div>
