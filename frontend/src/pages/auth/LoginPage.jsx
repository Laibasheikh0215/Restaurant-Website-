...
const handleSubmit = async (e) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/auth/social-login', {
        email, full_name: displayName || email.split('@')[0], provider: 'google', provider_id: uid,
      });
      if (response.data.success) {
        localStorage.setItem('token', response.data.token);
  ...
  const handleFacebookLogin = async () => {
    try {
      ...
      const { email, displayName, uid } = result.user;
      const response = await axios.post('http://localhost:5000/api/auth/social-login', {
        email, full_name: displayName || email.split('@')[0], provider: 'facebook', provider_id: uid,
      });
      if (response.data.success) {
        localStorage.setItem('token', response.data.token);
        toast.success(`Welcome ${response.data.user.full_name}!`);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="login-left" style={{
        ...
        {/* Glow orb */}
        <div style={{ position: 'absolute', top: '20%', right: '-80px', width: '320px', height: '320px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(232,68,26,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: '10%', left: '-60px', width: '240px', height: '240px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(232,68,26,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <h1 style={{ fontSize: 'clamp(32px, 3vw, 48px)', fontFamily: "'Playfair Display', serif", fontWeight: '700', color: '#fff', lineHeight: '1.1', marginBottom: '16px' }}>
            Your Table<br />
            <span style={{ color: '#E8441A', fontStyle: 'italic' }}>Awaits You</span>
          </h1>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.5)', lineHeight: '1.7', maxWidth: '340px' }}>
            Sign in to order food, reserve tables, and enjoy an unforgettable dining experience.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {FEATURES.map((f, i) => (
            <div key={f.text} className="feature-item" style={{ animationDelay: `${0.4 + i * 0.1}s`, display: 'flex', alignItems: 'center', gap: '14px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(232,68,26,0.1)', border: '1px solid rgba(232,68,26,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>
                {f.icon}
              </div>
      ...
      <div className="login-right" style={{
        ...
        <div style={{ marginBottom: '36px' }}>
          <div className="logo-pulse" style={{ fontSize: '42px', marginBottom: '16px' }}>🍽️</div>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: '28px', fontWeight: '700', color: '#fff', marginBottom: '8px' }}>
            Sign In
          </h2>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
          ...
          <div className="form-row-0" style={{ marginBottom: '14px' }}>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', letterSpacing: '1px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: '8px' }}>
              Email Address
            </label>
            <input
              type="email"
          ...
          {/* Password */}
          <div className="form-row-1" style={{ marginBottom: '10px', position: 'relative' }}>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', letterSpacing: '1px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: '8px' }}>
              Password
            </label>
            <div style={{ position: 'relative' }}>
              ...
              <button type="button" onClick={() => setShowPass(!showPass)}
                style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: 'rgba(255,255,255,0.35)' }}>
                {showPass ? '🙈' : '👁️'}
              </button>
            </div>
          </div>
          ...
          {/* Forgot password */}
          <div className="form-row-2" style={{ textAlign: 'right', marginBottom: '24px' }}>
        ...
        {/* Divider */}
        <div className="form-row-4" style={{ display: 'flex', alignItems: 'center', margin: '28px 0' }}>
          <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
          <span style={{ padding: '0 16px', fontSize: '12px', fontWeight: '600', letterSpacing: '2px', color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase' }}>or continue with</span>
          <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
        </div>
